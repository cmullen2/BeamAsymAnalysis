void CreateHtml6() {
  ...
  if (doBins) {
    ...
    string dirs[11] = {"Cut1", "Polp", "Polm","ProtonChannelPolm","ProtonChannelPolp","NeutronChannelPolm","NeutronChannelPolp","ProtonChannelPolmandCuts","ProtonChannelPolpandCuts","NeutronChannelPolmandCuts","NeutronChannelPolpandCuts"};
    ...
    for (int i=0; i<11; i++) {
      ...
      TKey 	*tbinKey;
      //CAM Alteration to create asymmetries
      if(i<10 && i>0){
	TDirectory *otherDir = infile->GetDirectory(dirs[i+1].c_str());
	TIter othernextTbinDir(otherDir->GetListOfKeys());
	TKey  *othertbinKey;
      }
	...
	TKey 	*ebinKey;
	if(i<10 && i>0){
	  othertbinKey=(TKey*)othernextTbinDir();
	  TDirectory *othertbinDir = otherDir->GetDirectory(othertbinKey->GetName());
	  TIter otherEbinDir(othertbinDir->GetListOfKeys());
	  TKey *otherebinKey;
	}
