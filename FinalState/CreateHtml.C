void CreateHtml() {
	...
	ofstream outHtml;
	outHtml.open("/home/chris/Docs/haspy/BeamAsymAnalysis/FinalState/outhtml/summary.html");
	...
	if (doBins) {
		...
		TFile *infile = TFile::Open("/w/work1/home/chris/LatestAnalysisRuns/Data/DataJul17/HaspectOutput/TestMulti/Dev15.0Files1PhysicsAll.root");
		TString dirs[7] = {"Cut1", "Polp", "Polm","ProtonChannelPolm","ProtonChannelPolp","NeutronChannelPolm","NeutronChannelPolp"};
		...
		for (int i=0; i<7; i++) {
			...
			TDirectory *weightsDir = infile->GetDirectory(dirs[i]);	
			...
			while ((tbinKey=(TKey*)nextTbinDir())) {
				...
				while ((ebinKey=(TKey*)nextEbinDir())) {
					...
					TKey	*histKey;
					cout << dirs[i] << " dirs " << endl;
					cout << tbinDir->GetName() <<endl;
					cout << ebinDir->GetName()<< endl;
					TString heading = dirs[i]+Form(" ")  +  Form("%s", (TString*)tbinDir->GetName())   + Form(" ")    +Form("%s",(TString*)ebinDir->GetName());
					cout << heading << endl;
					...
					while ((histKey=(TKey*)nextHist())) {
						...
						hist->Draw();
						TString imFileName = dirs[i]+Form("%s", (TString*)tbinDir->GetName()) + Form("%s",(TString*)ebinDir->GetName()) +  Form("%s", histKey->GetName())+ Form(".gif");
						cout << imFileName<<endl;
						can->SaveAs("/home/chris/Docs/haspy/BeamAsymAnalysis/FinalState/outhtml/"+imFileName);
						can->Clear();	
		...
		for (int histNum=0; histNum < histNames.size(); histNum++) {
			...
			for (int i=0; i<7; i++) {
				...
				while ((tbinKey=(TKey*)nextTbinDir())) {
					...
					if (writeHead) {
						while ((ebinKey=(TKey*)nextEbinDir())) {
							TString chris = Form("<TD><h1>") + Form("%s",(TString*)ebinKey->GetName() ) +Form("</h1>");
							outHtml << chris;
						}
						outHtml << endl;
						writeHead = false;
